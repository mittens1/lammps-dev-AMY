...
#include "pair_lj_cut_coul_cut_chunk.h"
...
PairLJCutCoulCutChunk::~PairLJCutCoulCutChunk()
{
  if (copymode) return;
...
void PairLJCutCoulCutChunk::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
...
void PairLJCutCoulCutChunk::settings(int narg, char **arg)
{
  ...
  /************************** TODO EVK ****************
  // Chunk computes come at the end
  if (narg == 3) {
    idchunk = utils::strdup(arg[1]);
    idcom = utils::strdup(arg[2]);
  } else {
    idchunk = utils::strdup(arg[2]);
    idcom = utils::strdup(arg[3]);
  }
  */
...
void PairLJCutCoulCutChunk::coeff(int narg, char **arg)
{
...
void PairLJCutCoulCutChunk::init_style()
{
...
void PairLJCutCoulCutChunk::allocate()
{
  allocated = 1;
...
double PairLJCutCoulCutChunk::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairLJCutCoulCutChunk::write_restart(FILE *fp)
{
...
void PairLJCutCoulCutChunk::read_restart(FILE *fp)
{
...
void PairLJCutCoulCutChunk::write_restart_settings(FILE *fp)
{
...
void PairLJCutCoulCutChunk::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairLJCutCoulCutChunk::write_data(FILE *fp)
{
...
void PairLJCutCoulCutChunk::write_data_all(FILE *fp)
{
...
double PairLJCutCoulCutChunk::single(int i, int j, int itype, int jtype, double rsq, double factor_coul,
...
void *PairLJCutCoulCutChunk::extract(const char *str, int &dim)
{
  dim = 2;
