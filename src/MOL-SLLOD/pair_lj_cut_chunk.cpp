...
PairLJCutChunk::PairLJCutChunk(LAMMPS *lmp) : Pair(lmp), molids(nullptr)
{
  respa_enable = 1;
...
void PairLJCutChunk::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      delcomz = com[index_i][2] - com[index_j][2];
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if (index_i != index_j) {
          ev_tally_chunk(i, j, nlocal, newton_pair, evdwl, 0.0, fpair, delx, dely, delz,
...
int PairLJCutChunk::pack_forward_comm(int n, int *list, double *buf, int /*pbc_flag*/,
                                            int * /*pbc*/)
{
  int i, j, m;
...
void PairLJCutChunk::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
...
void PairLJCutChunk::compute_inner()
{
  int i, j, ii, jj, inum, jnum, itype, jtype;
...
void PairLJCutChunk::compute_middle()
{
  int i, j, ii, jj, inum, jnum, itype, jtype;
  double xtmp, ytmp, ztmp, delx, dely, delz, fpair;
  double rsq, r2inv, r6inv, forcelj, factor_lj, rsw;
...
void PairLJCutChunk::compute_outer(int eflag, int vflag)
{
  int i, j, ii, jj, inum, jnum, itype, jtype;
  double xtmp, ytmp, ztmp, delx, dely, delz, evdwl, fpair;
  double rsq, r2inv, r6inv, forcelj, factor_lj, rsw;
...
void PairLJCutChunk::settings(int narg, char **arg)
{
...
void PairLJCutChunk::coeff(int narg, char **arg)
{
...
void PairLJCutChunk::init_style()
{
...
double PairLJCutChunk::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairLJCutChunk::write_restart(FILE *fp)
{
...
void PairLJCutChunk::read_restart(FILE *fp)
{
...
void PairLJCutChunk::write_restart_settings(FILE *fp)
{
...
void PairLJCutChunk::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
void PairLJCutChunk::write_data(FILE *fp)
{
...
void PairLJCutChunk::write_data_all(FILE *fp)
{
...
double PairLJCutChunk::single(int /*i*/, int /*j*/, int itype, int jtype, double rsq,
...
void PairLJCutChunk::born_matrix(int /*i*/, int /*j*/, int itype, int jtype, double rsq,
...
void *PairLJCutChunk::extract(const char *str, int &dim)
{
  dim = 2;
